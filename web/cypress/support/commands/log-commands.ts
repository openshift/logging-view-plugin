...
Cypress.Commands.add('grantLogViewRolesToUser', (index: string, project: string) => {
  ...
  if (Cypress.env('LOGIN_USERS')) {
    ...
    if (username != '') {
      ...
      cy.exec(
        `oc -n ${project} policy add-role-to-user cluster-logging-application-view ${username}`,
      );
...
Cypress.Commands.add('removeLogViewRolesFromUser', (index: string, project: string) => {
  ...
  if (Cypress.env('LOGIN_USERS')) {
    ...
    if (username != '') {
      ...
      cy.exec(
        `oc -n ${project} policy remove-role-from-user cluster-logging-application-view ${username}`,
        { failOnNonZeroExit: false },
      );
...
Cypress.Commands.add('assertLogsInLogsTable', () => {
  ...
  cy.getByTestId(TestIds.LogsTable).within(() => {
    ...
    cy.get('tr[data-test-rows="resource-row" ]').its('length').should('be.gt', 1);
  });
});
...
Cypress.Commands.add('assertFieldsInLogDetail', (indexFields: IndexField[]) => {
  cy.getByTestId(TestIds.LogsTable).within(() => {
    ...
    cy.byClass(Classes.LogDetail).within(() => {
      indexFields.forEach((field) => {
        if (!Cypress._.isEmpty(field.value)) {
          // skip if value is empty, null, or undefined
          let pattern = new RegExp(field.value);
          cy.contains('dt', field.name)
            .parent()
            .within(() => {
              cy.get('dd').invoke('text').should('match', pattern);
            });
        } else {
          cy.contains('dt', field.name);
        }
...
Cypress.Commands.add('assertInfraLogsInLogsTable', () => {
  //only check some of these index fileds
  const indexFields: IndexField = [{ name: 'openshift_log_type', value: 'infrastructure' }];
  cy.assertFieldsInLogDetail(indexFields);
});

Cypress.Commands.add('assertAuditLogsInLogsTable', () => {
  //only check some of these index fileds
  const indexFields: IndexField = [{ name: 'openshift_log_type', value: 'audit' }];
  cy.assertFieldsInLogDetail(indexFields);
});
...
Cypress.Commands.add('selectLogTenant', (tenant: string) => {
  ...
  cy.get('#logging-view-tenant-dropdown').contains('button', tenant).click();
...
Cypress.Commands.add('selectLogAttribute', (attribute: string) => {
  ...
  cy.byTestID(TestIds.AttributeFilters).contains('button', attribute).click();
  //close the dropdown list
  cy.get('body').click(0, 0);
});
...
Cypress.Commands.add('waitUntilLogsInLokiStack', (query?: string) => {
  cy.exec(`./cypress/support/scripts/findLogsInLokistack.sh '${query}'`, {
    failOnNonZeroExit: false,
  }).then(({ code, stdout }) => {
    cy.log(stdout);
