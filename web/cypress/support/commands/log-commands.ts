...
import {Classes,TestIds } from '../../../src/test-ids';
...
declare global {
  namespace Cypress {
    ...
    interface Chainable<Subject> {
      ...
      checkNamespaces(namespaces: string[]);
      checkPods(pods: string[]);
      checkContainers(containers: string[]);
...
Cypress.Commands.add('uiImpersonateUser', (rank: string) => {
  ...
  switch (String(Cypress.env('OPENSHIFT_VERSION'))) {
    ...
    case '4.21':
      ...
      break 
    default: 
      table_type="Users table"
      // aria-label="Users table" data-test="data-view-table" in 4.22
  }
  let username = "";
  if (Cypress.env('LOGIN_USERS')){
    username=Cypress.env('LOGIN_USERS').split(',')[rank_2_num[rank]].split(':')[0];
  ...
  cy.get(`table[aria-label="${table_type}"]`, { timeout: 120000 } ).should('exist');
...
Cypress.Commands.add('selectTenant', (tenant: string) => {
...
Cypress.Commands.add('checkOptions', (opts: string[]) => {
  opts.forEach((opt) => {
    cy.byTestID(TestIds.AttributeOptions)
      .find('input[type="text"]')  // find the input inside
      .click()                     // focus the input
      .clear()
      .type(opt);
    cy.byTestID(TestIds.AttributeFilters).within(() => {
      cy.get('[role="listbox"] li', { includeShadowDom: true })
        .should('have.length.greaterThan', 0)
        .each(($li) => {
          const text = $li.text().trim(); // remove spaces
          if (text === opt) {
            cy.wrap($li)
              .scrollIntoView()               // scroll into view if offscreen
...
Cypress.Commands.add('checkNamespaces', (namespaces: string[]) => {
  cy.checkOptions(namespaces);
})

//select pods from AttributeOptions
Cypress.Commands.add('checkPods', (pods: string[]) => {
  cy.checkOptions(pods);
})

//select Containers from AttributeOptions
Cypress.Commands.add('checkContainers', (containers: string[]) => {
  cy.checkOptions(containers);
})
...
Cypress.Commands.add('clickPodLink', (podName?: string) => {
  ...
  cy.getByTestId('data-view-table').within(() => {
    cy.get('td a')
