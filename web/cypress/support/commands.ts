...
declare global {
  namespace Cypress {
    interface Chainable<Subject> {
      ...
      allowViewProjectLogs(username: string, project: string);
...
Cypress.Commands.add('uiLoginAsClusterAdmin', (rank: string) => {
  ...
  if( username != "" && userpassword != "" && Cypress.env('LOGIN_IDP') != "" ){
    ...
    cy.log('Skipping login, no user can be found. the test continue.');
  }
});

//Allow user to view observe/logs,alerts
Cypress.Commands.add('allowViewProjectLogs', (username: string, project: string) => {
...
Cypress.Commands.add('uiLogout', () => {
  cy.window().then((win: any) => {
    ...
    cy.log('Log out UI');
    switch (Cypress.env('OPENSHIFT_VERSION')) {
      case "4.20":
      case "4.19":
	...
	cy.get('button.pf-v5-c-menu__item')
          .filter((i, el) => el.innerText.trim() === 'Log out')
          .click()
    }
  })
});
