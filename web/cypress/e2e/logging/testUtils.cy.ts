...
export function isDevConsoleReady(): boolean {
  ...
  if (major > 4 || (major === 4 && minor > 19)) {
    cy.exec("oc get console.operator cluster -o jsonpath='{.spec.customization.perspectives}'",{failOnNonZeroExit: false}).then((result) => {
...
export function aggrTest() {
  ...
  it('selected containers',{tags:['@aggr']}, () => {
    ...
    cy.selectAttribute('Containers');
    cy.checkContainers(containers);
...
export function observeTest() {
  it('selected namespaces',{tags:['@observ']}, () => {
    const namespaces=[APP_NAMESPACE1, APP_NAMESPACE2]
    cy.selectAttribute('Namespaces');
    cy.checkNamespaces(namespaces);
    cy.showQueryInput();
    cy.byTestID(TestIds.LogsQueryInput)
      .find('textarea')
      .invoke('val')
