...
export function isDevConsoleReady(): boolean {
  ...
  if (major > 4 || (major === 4 && minor > 18)) {
    return cy
      .exec("oc get console.operator cluster -o jsonpath='{.spec.customization.perspectives}'", {
...
export function aggrLogTest() {
      ...
      cy.byTestID(TestIds.LogsTable).within(() => {
        cy.get('td[data-label="message"]')
          ...
          .within(() => {
            ...
            cy.get(
              `a[href="/k8s/ns/${APP_NAMESPACE1}/pods/${pod1Name}/containers/centos-logtest"]`,
            ).should('exist');
          });
      });
    });
  ...
  it('selected containers', { tags: ['@aggr'] }, () => {
    ...
    if (String(Cypress.env('CLUSTERLOGGING_DATAMODE')) === 'otel') {
      pattern =
        /{ k8s_container_name="centos-logtest", k8s_pod_name=~"centos-logtest-\w+|centos-logtest-\w+" } /;
    }
    cy.byTestID(TestIds.LogsQueryInput).find('textarea').invoke('val').should('match', pattern);
    cy.byTestID(TestIds.ExecuteQueryButton).click();
    const indexFields: IndexField = [
      { name: 'k8s_namespace_name', value: `${APP_NAMESPACE1}` },
...
export function commonTest() {
  ...
  it('Search by content ', { tags: ['@common'] }, () => {
    ...
    cy.byTestID(TestIds.AttributeFilters).within(() => {
      cy.get('input[aria-label="Search by Content"]').clear().type('SVTLogger', { delay: 0 });
    });
    cy.showLogQueryInput();
  ...
  it('filter logs by custom range', { tags: ['@common'] }, () => {
    ...
    cy.byTestID(TestIds.TimeRangeSelectModal).within(() => {
      ...
      cy.get('input[aria-label="Precision time picker"]')
        .first()
        .clear()
        .type(`${startTime}{enter}`);
  ...
  it('validate Viaq log format for container', { tags: ['@common'] }, function () {
    ...
    cy.showLogQueryInput();
    cy.byTestID(TestIds.LogsQueryInput).find('textarea').invoke('val').should('include', '| json');

    cy.byTestID(TestIds.LogsTable)
      .should('exist')
      .within(() => {
        cy.get('td[data-label="date"]').first().invoke('text').should('match', timestampPattern);
        cy.get('td[data-label="message"]').first().invoke('text').should('match', viaqlogFormat);
      });
  ...
  it('validate Otel log format for container', { tags: ['@common'] }, function () {
    ...
    const timestampPattern = /^[A-Z][a-z]{2} \d{1,2}, \d{4}, \d{2}:\d{2}:\d{2}\.\d{3}$/;
    const otellogFormat =
      /^\{.*"@timestamp":".+?",.*"hostname":".+?",.*"kubernetes":\{.*\},.*"level":"\w+",.*"log_source":"container",.*"log_type":"application",.*"message":".+?",.*"openshift":\{.*\}.*\}$/;
    ...
    cy.byTestID(TestIds.LogsTable)
      ...
      .within(() => {
        ...
        cy.get('td[data-label="message"]').first().invoke('text').should('match', otellogFormat);
      });
  ...
  it('switch the dataFormat', { tags: ['@common'] }, function () {
    ...
    //default viaq
    cy.byTestID(TestIds.SchemaToggle).invoke('text').should('eq', 'viaq');
    cy.showLogQueryInput();
    cy.byTestID(TestIds.LogsQueryInput).find('textarea').invoke('val').should('include', '| json');
    cy.byTestID(TestIds.LogsTable).within(() => {
    ...
    cy.get('li').contains('button', 'otel').click();
    cy.showLogQueryInput();
    cy.byTestID(TestIds.LogsQueryInput)
      .find('textarea')
      .invoke('val')
    ...
    //switch back to Viaq
    cy.byTestID(TestIds.SchemaToggle).click({ force: true });
    cy.get('li').contains('button', 'viaq').click();
    cy.showLogQueryInput();
    cy.byTestID(TestIds.LogsQueryInput).find('textarea').invoke('val').should('include', '| json');
    cy.byTestID(TestIds.ExecuteQueryButton).click();
    cy.byTestID(TestIds.LogsTable)
