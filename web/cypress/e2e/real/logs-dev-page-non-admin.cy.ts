...
const DEV_MON_PAGE_URL = '/dev-monitoring/ns/log-test-app1/logs';
...
describe('Logs Page', () => {
  before( function() {
    //DevConsole is disabled in 4.20 by default. check if DevConsole is enabled for Openshift 4.20 and 4.21 
    if((String(Cypress.env('OPENSHIFT_VERSION')) == "4.19") || (String(Cypress.env('OPENSHIFT_VERSION')) == "4.20" ) || (String(Cypress.env('OPENSHIFT_VERSION')) == "4.21" )) {
    ...
    cy.switchToDeveloper();
  });

  beforeEach(() => {
    cy.visit(DEV_MON_PAGE_URL, { timeout: 120000 });
  });

  after( function() {
    if (!SKIPALL) {
      cy.uiLoginOut("second_user");
      cy.removeLogRoles("second_user", `${APP_NAMESPACE1}`)
      cy.removeLogRoles("second_user", `${APP_NAMESPACE2}`)
    }
  });

  it('display log top elements', () => {
  ...
  it('query applicatioins logs', () => {
    ...
    cy.getByTestId(TestIds.LogsQueryInput).within(() => {
      cy.get('textarea').clear().type(`{{}kubernetes_namespace_name="log-test-app1" {}} | json`, { delay: 0 });
    });
        ...
        cy.get('td.pf-v5-c-table__td.lv-plugin__table__details')
          ...
          .within(() => {
            ...
            if (String(Cypress.env('CLUSTERLOGGING_VERSION')) != "5.8" && String(Cypress.env('CLUSTERLOGGING_VERSION')) != "5.9" ) {
              if (String(Cypress.env('CLUSTERLOGGING_VERSION')) != "6.0" && String(Cypress.env('CLUSTERLOGGING_VERSION')) != "6.1" ) {
  ...
  it('forbidden infra container logs ', () => {
    cy.visit("/dev-monitoring/ns/openshift-apiserver/logs", { timeout: 120000 });
    cy.getByTestId(TestIds.LogsTable)
      .should('exist')
      .within(() => {
        cy.get('div.lv-plugin__table__row-error').should('contain', 'Forbidden');
        //cy.get('div.lv-plugin__table__row-error').should('contain', 'Forbidden');
      })
  });
  ...
  it('query with the selected content', () => {
    ...
    cy.getByTestId(TestIds.ShowQueryToggle).click();
    cy.get('div[data-test="LogsQueryInput"] textarea').should('have.value', `{ kubernetes_namespace_name="${APP_NAMESPACE1}" } |= ${BLACKTICK}${APP_NAMESPACE1}${BLACKTICK} | json`)
  ...
  it('query with the selected pods', () => {
    ...
    cy.get('div[data-test="LogsQueryInput"] textarea')
      .invoke('val')
      .should('match', new RegExp(`{{} kubernetes_namespace_name="${APP_NAMESPACE1}", kubernetes_pod_name=~"centos-logtest-\w+\|centos-logtest-\w+\|centos-logtest-\w+" {}} | json`))
  ...
  */
  ...
  it('show resources ', () => {
    ...
    cy.getByTestId(TestIds.ShowQueryToggle).click();
    cy.getByTestId(TestIds.LogsQueryInput).within(() => {
      cy.get('textarea').type(`{selectAll}`).type(`{{} kubernetes_namespace_name="${APP_NAMESPACE1}",kubernetes_pod_name="${pod1Name}"{}}`,{ delay: 0 });
