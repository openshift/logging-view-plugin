...
clusterlogging_csv=$(oc get csv -l "operators.coreos.com/cluster-logging.openshift-logging" -o jsonpath='{.items[0].metadata.name}')
...
if [[ $data_mode == "" ]];then
    export CYPRESS_CLUSTERLOGGING_DATAMODE="viaq"
fi
