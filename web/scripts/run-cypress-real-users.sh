...
#   prerequisite: 
#      Openshift-logging operators, lokistack, clusterloggingforward are deployed
#      Appplication, infrastructure and audit logs are sent to lokistack.
...
#      (In prow, you can call step openshift-observability-enable-cluster-logging to prepare test data)
#   At least two users must be provided via Environment CYPRESS_LOGIN_IDP,CYPRESS_LOGIN_USERS. The script can create htpasswd IPD and create five users uiauto-test-1..5 if the Environment is not defined
#   The Environment KUBECONFIG must be exported.
#   The test scenarios can be selected via Environment TEST_TO_RUN. when TEST_TO_RUN is null, all real cases will be executed.  TEST_TO_RUN="--spec xxx", TEST_TO_RUN="--tag xxx"
#
...
function enable_idp_htpasswd()
{
    ...
    first_record=${uitest_users%%,*}
    first_passwd=${first_record##*:}

    echo "oc login -u uiauto-test-1 -p <first_user_passwd> --config=/tmp/normal_kubeconfig"
    oc login  --username=uiauto-test-1 --password=${first_passwd} --kubeconfig=/tmp/normal_kubeconfig 1>/dev/null || exit 1
...
export CYPRESS_OPENSHIFT_VERSION=$(oc version -o json|jq  -r '.openshiftVersion' |cut -d. -f1,2 || exit1)
...
if [[ "$TEST_TO_RUN" == "" ]];then
    ...
    echo "## npx cypress run --e2e ${TEST_TO_RUN} --browser chrome"
    npx cypress run --e2e ${TEST_TO_RUN} --browser chrome
elif [[ "$TEST_TO_RUN"  =~ "--tag " ]]; then
    tags=${TEST_TO_RUN#*--tag }
    echo "## npx cypress --e2e run --env grep=${tags} --browser chrome"
    npx cypress run --e2e --env grep=${tags} --browser chrome
else
